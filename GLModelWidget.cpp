                                            ...
                                            const Imath::V3i& shift)
{
    ...
    SproxelGrid newGrid = SproxelGrid(newSize);
...
bool GLModelWidget::loadGridPNG(const std::string& filename)
{
    ...
    // Clear and load
    m_gvg=SproxelGrid(Imath::V3i(sizeX, sizeY, sizeZ));
...
bool GLModelWidget::saveGridPNG(const std::string& filename)
{
    ...
    for (int slice = 0; slice < cellDim.z; slice++)
    {
        ...
        for (int y = 0; y < cellDim.y; y++)
        {
            for (int x = 0; x < cellDim.x; x++)
            {
                const Imath::Color4f& colorScaled = m_gvg.get(Imath::V3i(x, y, slice)+dim.min) * 255.0f;
...
bool GLModelWidget::exportGridOBJ(const std::string& filename, bool asTriangles)
{
    ...
    for (int y = 0; y < sy; y++)
    {
        for (int z = 0; z < sz; z++)
        {
            for (int x = 0; x < sx; x++)
            {
                const Imath::Color4f& color = m_gvg.get(Imath::V3i(x, y, z)+dim.min);
    ...
    for (int y = 0; y < (sy+1); y++)
    {
        for (int z = 0; z < (sz+1); z++)
        {
            ...
            {
                int neighbors = 0;
                if ((x!=0)  && (y!=0)  && (z!=0)  && (m_gvg.get(Imath::V3i(x-1, y-1, z-1)+dim.min).a != 0.0f)) neighbors++;
                if ((x!=0)  && (y!=0)  && (z!=sz) && (m_gvg.get(Imath::V3i(x-1, y-1, z  )+dim.min).a != 0.0f)) neighbors++;
                if ((x!=0)  && (y!=sy) && (z!=0)  && (m_gvg.get(Imath::V3i(x-1, y,   z-1)+dim.min).a != 0.0f)) neighbors++;
                if ((x!=0)  && (y!=sy) && (z!=sz) && (m_gvg.get(Imath::V3i(x-1, y,   z  )+dim.min).a != 0.0f)) neighbors++;
                if ((x!=sx) && (y!=0)  && (z!=0)  && (m_gvg.get(Imath::V3i(x,   y-1, z-1)+dim.min).a != 0.0f)) neighbors++;
                if ((x!=sx) && (y!=0)  && (z!=sz) && (m_gvg.get(Imath::V3i(x,   y-1, z  )+dim.min).a != 0.0f)) neighbors++;
                if ((x!=sx) && (y!=sy) && (z!=0)  && (m_gvg.get(Imath::V3i(x,   y,   z-1)+dim.min).a != 0.0f)) neighbors++;
                if ((x!=sx) && (y!=sy) && (z!=sz) && (m_gvg.get(Imath::V3i(x,   y,   z  )+dim.min).a != 0.0f)) neighbors++;
    ...
    for (int y = 0; y < (sy+1); y++)
    {
        for (int z = 0; z < (sz+1); z++)
        {
            for (int x = 0; x < (sx+1); x++)
            {
                ...
                const Imath::M44d mat = m_gvg.voxelTransform(voxelToCheck+dim.min);
    ...
    for (int y = 0; y < sy; y++)
    {
        for (int z = 0; z < sz; z++)
        {
            for (int x = 0; x < sx; x++)
            {
                ...
                if (x == 0)
                    crossNegX = true;
                else if (color.a != m_gvg.get(Imath::V3i(x-1,y,z)+dim.min).a)
                    crossNegX = true;
                ...
                if (y == 0)
                    crossNegY = true;
                else if (color.a != m_gvg.get(Imath::V3i(x,y-1,z)+dim.min).a)
                    crossNegY = true;
                ...
                if (z == 0)
                    crossNegZ = true;
                else if (color.a != m_gvg.get(Imath::V3i(x,y,z-1)+dim.min).a)
                    crossNegZ = true;
